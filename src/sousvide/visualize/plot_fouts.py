...
def plot_rollout_data(cohort:str,Nsamples:int=50,random:bool=True):
    ...
    for course in os.listdir(rollout_path):
        ...
        trajs_path = os.path.join(trajs_path, trajs_file)
        
        # Load the trajectories
        trajectories = torch.load(trajs_path)

        # Trim the number of samples
        if Nsamples > len(trajectories):
            Nsamples = len(trajectories)
            print(f"Only {Nsamples} samples available in {trajs_file}. Showing all samples.")
        else:
            print(f"Showing {Nsamples} samples from {trajs_file}")

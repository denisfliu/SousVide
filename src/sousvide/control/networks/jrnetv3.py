...
class JRNetv3(BaseNet):
    def __init__(self,
        ...
        super(JRNetv3, self).__init__(inputs,outputs,network_type)
    ...
    def forward(self,
        ...
        mem_mask = torch.triu(torch.full((Nwd_st, Nwd_in), float('-inf')), diagonal=1)
        tgt_mask = tgt_mask.to(xnn_st.device)
        mem_mask = mem_mask.to(xnn_st.device)
        
        znn = self.networks["decoder"](tgt, mem, tgt_mask=tgt_mask, memory_mask=mem_mask)
